    ...
    <div class="form_wrapper">
      <form class="mt-4" @submit.prevent="validateFormInputs" autocomplete="off">
        <div class="row justify-content-center">
          <!-- Start:: Email Input -->
          <base-input col="7" type="email" :placeholder="$t('PLACEHOLDERS.email')" v-model.trim="loginData.email"
            required />
          <!-- End:: Email Input -->
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        localStorage.setItem("dalilkum_provider_roles", JSON.stringify(res.data.data.user));

        console.log(JSON.parse(localStorage.getItem("dalilkum_provider_roles")));
