    ...
    <div class="form_wrapper">
      <form class="mt-4" @submit.prevent="validateFormInputs">
        <div class="row justify-content-center">
          <!-- Start:: Email Input -->
          <base-input col="7" type="email" :placeholder="$t('PLACEHOLDERS.email')"
            v-model.trim="resetPassPhoneNumData.email" required />
          <!-- End:: Email Input -->
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        localStorage.setItem("dalilkum_provider_dashboard_verification_code", res.data.data.code);
        localStorage.setItem("dalilkum_provider_dashboard_forget_pass_token", res.data.data.access_token);
