  ...
  <div class="show_all_content_wrapper">
    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.additional_field')"
      ...
      <div class="table_title_wrapper">
        ...
        <div class="title_route_wrapper" v-if="$can('additionalFields create', 'additionalFields')">
          <router-link to="/additionalFields/create">
            {{ $t("PLACEHOLDERS.add_additional_fields") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.realEstateDepartments`]="{ item }">
          <h6 class="text-danger" v-if="!item.realEstateDepartments"> {{ $t("TABLES.noData") }} </h6>
          <div v-else v-for="(item, index) in item.realEstateDepartments" :key="'p' + index">
        ...
        <template v-slot:[`item.is_required`]="{ item }">
          <div v-if="item.is_required == 0">{{ $t('PLACEHOLDERS.optional') }}</div>
          <div v-else>{{ $t('PLACEHOLDERS.required_item') }}</div>
        </template>

        <!-- Start:: Activation -->
        <template v-slot:[`item.is_active`]="{ item }">
          <div class="activation" dir="ltr" style="z-index: 1"
            v-if="$can('additionalFields activate', 'additionalFields')">
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalFields show', 'additionalFields')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalFields edit', 'additionalFields')">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('additionalFields delete', 'additionalFields')">
              <template slot="title">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.additional_field"),
          value: "name",
          sortable: false,
          align: "center",
        },
        ...
        {
          text: this.$t("PLACEHOLDERS.Adjective"),
          value: "is_required",
          sortable: false,
          align: "center",
        },
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            status: this.filterOptions.is_active?.id,
          },
        });
        this.loading = false;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "GET",
          url: `additionalFields/active/${item.id}`,
        });
        this.setTableRows()
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/additionalFields/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/additionalFields/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `additionalFields/${this.itemToDelete.id}`,
        });
