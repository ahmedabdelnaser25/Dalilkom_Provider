        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              />
              <!-- End:: phone Input -->
              <!-- Start:: companyName Input -->
              <base-input
                col="4"
                type="text"
                :placeholder="$t('PLACEHOLDERS.company_name')"
                v-model.trim="filterOptions.companyName"
              />
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('wallets create', 'wallets')"
            >
              <template slot="title">
                <span>{{ $t("BUTTONS.show") }}</span>
              </template>
              <button class="btn_show" @click="selectAcceptedItem(item)">
                <span class="text-success text-h5">
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            user: this.filterOptions.title,
            number: this.filterOptions.phone,
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `withdraw-requests/status/${item.id}`,
          data: REQUEST_DATA,
        });
        this.$message.success(this.$t("MESSAGES.changeActivation"));
      } catch (error) {
