    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.providers-updates") }}</h5>
          <button
...
export default {
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/providers-updates/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      ...
      try {
        await this.$axios.post(
          `profile-update-request/${this.itemToDelete.id}`,
          REQUEST_DATA
        );
    ...
    async approvedProviders(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `profile-update-request/${item.id}`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
