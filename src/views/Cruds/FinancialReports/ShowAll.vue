    ...
    <main>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.total_orders`]="{ item }">
           <router-link :to="`/orders/all?page=1&provider=${item.id}&type=finished`">
            {{ item.total_orders }}
          </router-link>
        </template>
  ...
  methods: {
    ...
    async setTableRows() {
      ...
      try {
        let res = await this.$axios({
          method: "GET",
          url: "financials?type=not_finished",
          params: {
    ...
    async confirmPdf() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `financials/export-pdf?type=not_finished`,
        });
    ...
    async confirmExcel() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `financials/export-excel?type=not_finished`,
        });
