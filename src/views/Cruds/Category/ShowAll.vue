    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.main_categories") }}</h5>
          <button
        ...
        >
          <router-link to="/category/create">
            {{ $t("PLACEHOLDERS.add_new_section") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/category/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/category/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/category/show/${item.id}` });
    },
    // ===== End:: End
