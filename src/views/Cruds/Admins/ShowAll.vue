      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('provider_admins edit', 'provider_admins') && item.id !== 1"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('provider_admins delete', 'provider_admins') && item.id !== 1"
            >
              <template slot="title">
            ...
            <template v-if="$can('provider_admins activate', 'provider_admins') && item.id !== 1">
  ...
  methods: {
    ...
    async HandlingItemActivationStatus(selectedItem) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `admins/status/${targetItem.id}`,
          data: REQUEST_DATA,
        });
