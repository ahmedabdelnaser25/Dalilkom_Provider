    ...
    <main>
          ...
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <base-input
                col="6"
                type="text"
                :placeholder="$t('PLACEHOLDERS.measure_name')"
      ...
      <div class="table_title_wrapper">
        ...
        <div
          ...
          v-if="$can('weights create', 'weights')"
        >
          <router-link to="/measure/create">
            {{ $t("PLACEHOLDERS.add_new_measure") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('weights activate', 'weights')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('weights delete', 'weights')"
            >
              <template slot="title">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.measure_name"),
          value: "name",
          sortable: false,
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/measure/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/measure/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/measure/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `weights/${this.itemToDelete.id}`,
        });
