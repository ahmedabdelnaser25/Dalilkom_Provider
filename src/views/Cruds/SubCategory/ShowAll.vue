    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.app_sub_sections") }}</h5>
          <button
        ...
        <div
          ...
          v-if="$can('sub_categories create', 'sub_categories')"
        >
          <router-link to="/sub-category/create">
            {{ $t("PLACEHOLDERS.add_new_sub_section") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('sub_categories activate', 'sub_categories')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('sub_categories show', 'sub_categories')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('sub_categories edit', 'sub_categories')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('sub_categories delete', 'sub_categories')"
            >
              <template slot="title">
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.sub_section"),
          value: "name",
          align: "center",
          sortable: false,
        },
        {
          text: this.$t("PLACEHOLDERS.main_category_name"),
          value: "category.name",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `sub-categories/status/${item.id}`,
          data: REQUEST_DATA,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/sub-category/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/sub-category/show/${item.id}` });
    },
    // ===== End:: End
