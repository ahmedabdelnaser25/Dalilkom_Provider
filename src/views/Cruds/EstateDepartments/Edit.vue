<template>
  <div class="crud_form_wrapper">
    ...
    <div class="form_title_wrapper">
      <h4>{{ $t("PLACEHOLDERS.edit_property_section") }}</h4>
    </div>
    <!-- End:: Title -->
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `real-estate-departments/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
        this.$message.success(this.$t("MESSAGES.addedSuccessfully"));
        this.$router.push({ path: "/estate-departments/all" });
      } catch (error) {
        this.isWaitingRequest = false;
    ...
    async showData() {
      try {
        ...
        this.data.nameAr = res.data.data.RealEstateDepartment.name_ar;
        this.data.nameEn = res.data.data.RealEstateDepartment.name_en;
        this.data.active = res.data.data.RealEstateDepartment.is_active;
      } catch (error) {
        this.loading = false;
